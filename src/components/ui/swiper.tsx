  ...
  return (
    ...
    >
      {MarketPlaceCardData.map((data) => {
        ...
        return (
          <SwiperSlide className='' key={data.id} style={{ width: 400, position: "relative", height: 350, borderRadius: 10, backgroundColor: "#fff", border: "2px solid #FA78FF" }}>
            <CardStack items={data.images} />
            <div className='group flex px-4 items-center capitalize gap-4 text-[#CEB9E9] mt-2 py-4 absolute border-t rounded-t-lg border-[#FA78FF] w-full top-full left-0 -translate-y-full bg-gradient-to-br from-[#291D31] from-[0%] via-[190F1F] via-[74%] to-[#190F1F] to-[99%]'>
              ...
              <div className='flex w-full flex-col justify-center gap-2 items-start'>
                <p className='capitalize text-sm flex gap-1 font-semibold text-[#625B71] justify-center items-center'>{data.name.split(" ")[0]}.Tease <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-4">
                ...
                <div className='group flex-col gap-4 w-full transition-all ease-in-out  items-center justify-between'>
                  <div className='group-hover:hidden flex justify-between w-full gap-4 '>
                    <span className='flex gap-1 text-sm items-center text-[#625B71] font-light'>
                      <svg
                    ...
                    <span className=' flex gap-1 items-center text-sm text-[#625B71]  font-light'>
                      <svg
                        xmlns='http://www.w3.org/2000/svg'
                        fill='none'
                        viewBox='0 0 24 24'
                  ...
                  <div
                    ...
                    <Link href={`/profile/${data.slug}`}>
                      <Button
                        className='w-full flex h-[30px] text-sm items-center justify-center'
                      >
                        View full profile
                      </Button>
                    </Link>
